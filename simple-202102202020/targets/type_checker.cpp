...
void simple::type_checker::processUnaryExpression(cdk::unary_operation_node *const node, int lvl) {
...
void simple::type_checker::processBinaryExpression(cdk::binary_operation_node *const node, int lvl) {
  ASSERT_UNSPEC;
...
void simple::type_checker::do_variable_node(cdk::variable_node *const node, int lvl) {
  ASSERT_UNSPEC;
...
void simple::type_checker::do_assignment_node(cdk::assignment_node *const node, int lvl) {
  ...
  try {
    ...
    auto symbol = std::make_shared<simple::symbol>(cdk::primitive_type::create(4, cdk::TYPE_INT), id, 0);
