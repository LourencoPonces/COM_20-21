...
static std::string boolean_name(bool boolean) {
...
void fir::xml_writer::do_leave_node(fir::leave_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " nCycles='" << node->nCycle() << "'>" << std::endl;
  closeTag(node, lvl);
}
...
void fir::xml_writer::do_write_node(fir::write_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " new-line='" << boolean_name(node->newL()) << "'>" << std::endl;
  if (node->arguments()) {
...
void fir::xml_writer::do_variable_definition_node(fir::variable_definition_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " identifier='" << node->identifier() << "' qualifier='"
...
void fir::xml_writer::do_function_declaration_node(fir::function_declaration_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  if (_inFunctionArgs) {
    error(node->lineno(), "cannot define function in args");
    return;
  }
...
void fir::xml_writer::do_function_definition_node(fir::function_definition_node *const node, int lvl) {
  ...
  if(node->default_return_value()) {
    openTag("default-return-value", lvl + 2);
    node->default_return_value()->accept(this, lvl + 4);
    closeTag("default-return-value", lvl + 2);
  }
